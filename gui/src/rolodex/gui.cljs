...
(defn request [method uri opts]
  (swap! app-state assoc :last-request [method uri])
...
(defn submit-contact! [id]
  (let [c (find-contact id)
        path (str "/api/" id)]
    (if (:unsaved c)
      (POST path {:format prn-str :params c})
      (PUT path {:format prn-str :params c}))))
...
(defn Card [contact editing]
  [:div.card
   [:div.toprow [:button {:disabled (not contact)
                          :on-click (fn []
                                      (when editing (submit-contact! (:id contact)))
                                      (toggle-edit!))}
                 (if editing "✔️" "✏️")] [:span.uuid (str (:id contact))]]
